...
- [ ] **Choose hosting for both processes** (see `RUNBOOK.md` → Deploying) — they are not
  ...
  - `apps/worker` — needs a **long-lived container**, not a serverless/request-scoped runtime, since
    ...
    host this half. Railway is a straightforward fit (this project's actual choice, alongside
    `apps/web` on Vercel) - `.railway/railway.ts` is ready too, deployed via the Railway CLI's
    `railway config plan`/`apply`, not the dashboard; `RUNBOOK.md` → "Deploying the worker to
    Railway" has the step-by-step.
